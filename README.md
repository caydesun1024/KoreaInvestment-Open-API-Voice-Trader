

# V-HTS 2.0 — Voice Hybrid Trading System

> 음성 명령 한 마디로 주식을 거래하는 AI 기반 금융 에이전트 프로토타입

---

## 목차

- [개요](#개요)
- [현재 구현 기능](#현재-구현-기능)
- [시스템 아키텍처](#시스템-아키텍처)
- [하이브리드 의도 분석 파이프라인](#하이브리드-의도-분석-파이프라인)
- [기술 스택](#기술-스택)
- [프로젝트 구조](#프로젝트-구조)
- [환경 변수 설정](#환경-변수-설정)
- [실행 방법](#실행-방법)
- [로드맵](#로드맵)
- [트러블슈팅 및 성능 최적화](#트러블슈팅-및-성능-최적화-기록-2026-03-07)

---

## 개요

**V-HTS 2.0**은 자연어 음성 명령을 분석하여 한국투자증권(KIS) Open API로 실제 매매를 수행하는 AI 트레이딩 에이전트입니다.

단순 명령어 매칭이 아닌, **3단계 하이브리드 의도 분석 파이프라인(Regex → RapidFuzz → Qwen)**을 통해 "SK하이닉스 좀 사줘", "삼성전자 지금 얼마야?" 같은 자연스러운 표현을 그대로 처리합니다. 정형화된 요청은 Regex와 퍼지 매칭으로 빠르게 처리하고, 복잡한 의도만 LLM으로 넘겨 응답 속도와 비용을 함께 최적화했습니다.

> **개발 노트:** 초기에는 Qwen-2.5-7B를 종목 데이터로 파인튜닝(LoRA/PEFT)하여 자체 판단 모델을 만들려 했으나, 학습 정확도는 높았음에도 실제 다양한 발화에서는 제대로 동작하지 않는 과적합 문제를 겪었습니다. 원인 분석 끝에 RAG 방식(종목명-코드 매핑 데이터 기반)으로 전환해 안정적인 정확도를 확보했습니다.

---

## 현재 구현 기능

| 기능 | 상태 | 비고 |
|------|:----:|------|
| 음성 입력 (마이크 녹음) | ✅ | 웹 UI에서 직접 녹음 |
| STT 변환 | ✅ | RTZR VITO API |
| 텍스트 직접 입력 | ✅ | `/ask` 엔드포인트 |
| 종목명 퍼지 매칭 | ✅ | RapidFuzz, 2500+ 종목 |
| 의도 분석 (3단계 하이브리드) | ✅ | Regex → RapidFuzz → Qwen (RAG) |
| 주가 조회 | ✅ | KIS REST API (모의/실전), Redis 캐싱 |
| 차트 데이터 (15분봉/일봉) | ✅ | Redis 캐싱 적용, 9시 데이터 무결성 보장 |
| 계좌/포트폴리오 조회 | ✅ | Redis 캐싱 적용, 지연 최적화 |
| 시장가 매수 주문 | ✅ | KIS API, MCP 기반 함수 라우팅 |
| 웹 대시보드 UI | ✅ | Next.js (TypeScript) + Tailwind + Recharts |
| 매도 주문 | ❌ | 로드맵 |
| 체결 통보 (실시간) | ❌ | 로드맵 |
| TTS 피드백 | ❌ | 로드맵 |

---

## 시스템 아키텍처

```
[사용자 음성]
      │
      ▼
[RTZR VITO STT]  ◄─────────────────────────────┐
      │                                         │
      ▼                                   [웹 UI]
[3단계 의도 분석: Regex→RapidFuzz→Qwen(RAG)]  ◄── 텍스트 직접 입력도 가능 ──┘
      │
      ├── action: inquiry ──► [Redis Cache] ◄──► [KIS REST API]
      │                              │
      └── action: buy ──────► [MCP: 자연어 의도 → KIS 함수 매칭/호출]
                                      │
                              [JSON 응답 → 웹 UI 표시]
```

---

## 하이브리드 의도 분석 파이프라인

1. **Regex** — 정형화된 명령(종목명+수량+매수/매도 동사 조합)을 우선 처리. "사줘", "팔아줘" 등 다양한 매수매도 표현 패턴을 실제 발화 데이터 기반으로 반영
2. **RapidFuzz** — 종목명 표기가 정확하지 않거나 축약된 경우, 2,500개 이상 종목 리스트와 퍼지 매칭으로 보정
3. **Qwen 2.5B (RAG)** — 위 두 단계로 처리되지 않는 복잡한 자연어 요청만 LLM으로 라우팅. 종목명-코드 매핑 데이터를 RAG로 주입해 정확도 확보 (파인튜닝 대신 RAG를 채택한 배경은 [개요](#개요) 참고)

**MCP(Model Context Protocol)**는 Qwen이 파악한 사용자 의도를 실제 KIS API 함수(시세 조회, 매수 주문 등)로 매칭·호출하는 함수 라우팅 계층으로 사용했습니다.

---

## 트러블슈팅 및 성능 최적화 기록 (2026-03-07)

### 🚀 Redis 캐시 레이어 도입을 통한 성능 혁신
* **문제**: 모든 주식 데이터 조회 시 KIS API 네트워크 지연(평균 100ms~200ms) 발생 및 TPS 제한으로 인한 서비스 불안정.
* **해결**: `Redis`를 도입하여 핵심 데이터에 전략적 캐싱 적용.
  * **현재가**: 60초 캐싱 (속도 약 **600배** 향상: 100ms → **0.1ms**)
  * **계좌/자산**: 5~10초 캐싱 (검색 시 0원이 표시되는 딜레이 현상 제거)
  * **차트 데이터**: 5분~1시간 캐싱 (첫 로딩 후 즉시 응답 가능)

### 📉 KIS API "시간 역행(Wrap-around)" 버그 해결
* **현상**: 15분봉 조회 시 09:00 데이터까지 오지 않고 14시 부근에서 끊기거나 전날 데이터가 섞이는 현상.
* **원인**: KIS API에 `09:00` 데이터를 요청하면, 모자란 데이터를 **전날 오후 15:30 데이터로 채워주는** 기이한 동작 확인.
* **해결**: 배치의 `oldest > newest` (시간 역전) 감지 로직 구현. 날짜 경계를 넘는 순간 수집을 중단하고 오늘 데이터만 필터링하여 **정확히 9시 정각 데이터까지의 무결성 보장**.

### 🖱️ 차트 스크롤 및 UX 조작성 개선
* **스크롤 차단**: Native Event Listener의 `{ passive: false }` 옵션으로 차트 줌 조작 시 페이지 전체가 스크롤되는 간섭 현상 완전 차단.
* **드래그 감도**: 차트 컨테이너 픽셀 너비 대비 데이터 포인트 비율을 계산하여 **실제 드래그 거리와 차트 이동 거리를 1:1로 매칭** (Panning 최적화).
* **강제 동기화**: Redis 캐시가 꼬였을 경우를 대비해 **5초 쿨타임이 적용된 Force Refresh(Sync)** 버튼 구현.

### 🛠️ KIS API 제약 사항 극복
* **TPS 에러 방어**: API 호출 간 **Staggering(미세 지연)** 적용 및 **"불굴의 재시도(Relentless Retry)"** 로직으로 TPS 초과 에러(`EGW00201`) 발생 시 자동 복구.
* **서버 제약 해결**: 모의투자 환경에서 `chk_holiday` 호출 불가 문제를 **실전 서버 강제 할당** 방식으로 해결.

### 🧠 LLM 파인튜닝 시도와 RAG로의 전환
* **시도**: Qwen-2.5-7B를 종목 데이터로 LoRA(PEFT) 파인튜닝해 자체 판단 모델을 구축.
* **문제**: 학습 정확도는 높게 나왔지만, 실제 다양한 발화 패턴에서는 제대로 동작하지 않는 과적합 현상 발생.
* **해결**: 파인튜닝 대신 종목명-코드 매핑 데이터를 활용한 **RAG 방식**으로 전환. 별도 학습 없이도 안정적인 종목 인식 정확도 확보.

---

## 기술 스택

| 분야 | 기술 |
|------|------|
| **Backend** | Python 3.13, FastAPI, asyncio |
| **Database** | PostgreSQL, Redis (Cache) |
| **AI 모델** | Qwen-2.5-7B-Instruct + RAG (종목명-코드 매핑) |
| **의도 분석** | Regex, RapidFuzz |
| **STT** | RTZR VITO API |
| **에이전트/함수 라우팅** | Claude Code + MCP |
| **Frontend** | Next.js 15, TypeScript, Recharts, Lucide Icons |
| **Infra (시도)** | OCI + Terraform (GPU 인스턴스 프로비저닝 자동화 시도 → 리전 재고 부족으로 로컬 전환) |
| **Package Mgr** | uv (Fast Python Package Installer) |

---

## 프로젝트 구조

```
voice-trader/
├── main.py                          # FastAPI 서버 진입점
├── benchmark_redis.py               # Redis 성능 측정 스크립트
├── server_stop.sh                   # 정밀 타격형 서비스 종료 스크립트
│
├── src/
│   ├── api/
│   │   ├── kis_auth.py              # KIS 인증
│   │   └── domestic_stock_functions.py  # KIS REST API 함수 모음 (MCP 라우팅 대상)
│   ├── services/
│   │   └── trading_service.py      # Redis 캐시 로직이 통합된 핵심 서비스
│   └── utils/
│       ├── redis_client.py         # Redis 연결 및 DataFrame 직렬화 유틸
│       └── mapper.py               # 종목명 → 코드 매핑 (RAG 참조 데이터)
│
└── v0-project/                      # Next.js 프론트엔드 프로젝트
```

---

## 로드맵

### Phase 1 — 트레이딩 기능 완성 (진행 중)

- [x] Redis 기반 성능 최적화 및 캐싱 시스템 구축
- [x] 9시 장 시작 데이터 무결성 확보
- [x] 파인튜닝 → RAG 전환으로 종목 인식 정확도 확보
- [ ] 매도 주문 ("삼성전자 10주 팔아")
- [ ] 잔고 및 보유 종목 조회 UI 고도화

### Phase 2 — 보안 및 멀티 유저 (Next)

- [ ] JWT 기반 로그인 시스템
- [ ] 사용자별 KIS API 키 암호화 저장
- [ ] PostgreSQL 스키마 확장 (영구 데이터 저장)

### Phase 3 — 안전성 및 사용성

- [ ] TTS 체결 피드백 ("삼성전자 10주 매수 완료")
- [ ] 리스크 게이트 (일일 손실 한도 설정)
